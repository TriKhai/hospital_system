...
const appointmentService = {
  ...
  getForDoctor: async ():Promise<AppointmentResponse[]> => {
    const res = await axiosClient.get<ResponseData<AppointmentResponse[]>>(`${BASE_URL}/doctor`);
    return res.data.data;
  },
  ...
  cancelByDoctor: async (id: number): Promise<void> => {
    const res = await axiosClient.patch<ResponseData<void>>(`${BASE_URL}/${id}/cancel-by-doctor`);
    return res.data.data;
  },
  confirmByDoctor: async (id: number): Promise<void> => {
    const res = await axiosClient.patch<ResponseData<void>>(`${BASE_URL}/${id}/confirm-by-doctor`);
    return res.data.data;
  }
};
